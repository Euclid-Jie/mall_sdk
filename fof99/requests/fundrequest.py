...
class FundAdvancedList(BaseRequest):
    ...
    def set_params(self, strategy_one, strategy_two='不限', strategy_three='不限', type_=1,
                   page=1, pagesize=10, order_by='price_date', order=1, fund_state=1, fund_type=None):
...
class PersonalFundPrice(BaseRequest):
    ...
    def set_params(self, fid, start_date=None, end_date=None, order_by='price_date', order=1, per_type=1):
        self['fid'] = fid
        if start_date:
            self['start_date'] = start_date
        if end_date:
...
class FofSubFundTrackList(BaseRequest):
    ...
    def set_params(self, type_, page=1, page_size=10, user_pool=""):
        self['type'] = type_
        self['page'] = page
        self['pagesize'] = page_size
        if user_pool != "":
...
class ValuationValuationList(BaseRequest):
    _uri = "/valuation/ValuationList"
